                            ...
                            if ($row2['balance'] > $abl) {
                                ...
                                if (mysqli_num_rows($query3) > 0) {
                                    while ($row3 = mysqli_fetch_assoc($query3)) {
                                        ...
                                        $sql4 = "INSERT INTO send_money(`number`, `number2`, `tk`, `transaction`, `ring`, `date`) VALUES ('{$phone}', '{$number}', '{$amount}', '{$tr}', '{$ring}', '{$d}')";
                                        $query4 = mysqli_query($con, $sql4) or die('query unsucessful');

                                        header('Location: ./send_succ.php');
                                    }
                                } else {
