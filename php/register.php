...
if ( isset( $_POST[ 'rsub' ] ) ) {
    ...
    if ( empty( $name || $phone || $email || $password || $gender || $address ) ) {
        ...
        if ( mysqli_num_rows( $query1 ) > 0 ) {
            ...
            $sqqqll = "INSERT INTO dps(`dhoron`, `meyad`, `joma`, `name`, `number`, `amount`, `i`, `date`) VALUES ('NO','NO','NO','{$name}','{$phone}','0','0','{$d}')";
            $queryll = mysqli_query( $con, $sqqqll ) or die( 'Query unsucessful' );
